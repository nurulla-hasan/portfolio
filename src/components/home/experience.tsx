...
const EXPERIENCE_DATA = [
  ...
  //   description: "Leading the development of high-performance architectural web applications. Implementing complex state management and microservices communication.",
  ...
  //   duration: "2021 - 2023",
  //   description: "Built scalable e-commerce platforms and automated deployment pipelines. Focused on UI/UX precision and backend optimization.",
  ...
  // },
  {
    ...
    location: "Dhaka, BD",
    duration: "2019 - 2021",
    description: "Assisted in building internal ERP tools and client-facing dashboards. Mastered the fundamentals of the MERN ecosystem.",
...
export default function Experience() {
  return (
    <section id="experience" className="relative min-h-screen flex items-center overflow-hidden bg-transparent">
      ...
      <SectionLayout className="relative z-10">
        ...
        <div className="mb-10 md:mb-16">
          ...
          <h2 className="text-[clamp(2.5rem,6vw,4rem)] leading-[0.9] font-bold tracking-wide uppercase text-foreground">
            Experience <span className="text-sm font-mono text-muted-foreground tracking-widest">[RECORD_03]</span>
          </h2>
        </div>
        ...
        <div className="relative">
          ...
          <div className="absolute left-0 md:left-1/2 top-0 bottom-0 w-px bg-border/40 -translate-x-1/2 overflow-hidden">
             <div className="absolute top-0 left-0 w-full h-20 bg-linear-to-b from-transparent via-primary to-transparent animate-[walk-v_5s_linear_infinite]" />
          </div>
            ...
            {EXPERIENCE_DATA.map((exp, idx) => (
              ...
              >
                ...
                <div className="absolute left-0 md:left-1/2 -translate-x-1/2 z-20">
                   <div className={`w-4 h-4 rounded-full border-2 ${exp.status === 'CURRENT_ACTIVE' ? 'border-primary bg-primary shadow-[0_0_12px_var(--primary)]' : 'border-primary/40 bg-background'} transition-all duration-500`}>
                ...
                <div className="w-full md:w-[45%] pl-8 md:pl-0">
                  <div className={`glass-terminal p-8 rounded-2xl relative group overflow-hidden border border-border/40 hover:border-primary/30 transition-all duration-500 bg-background/30 backdrop-blur-sm`}>
                    {/* Background Sweep */}
                    ...
                    <div className="mb-6">
                      <div className="flex items-center justify-between mb-2">
                         <span className="text-[0.6rem] font-bold tracking-[0.3em] uppercase opacity-40 group-hover:opacity-100 transition-opacity duration-500 text-primary">SEQ_{idx + 1}</span>
                         <span className="text-[10px] font-mono text-muted-foreground/40">{idx + 1}.0_XP</span>
                      </div>
                      <h3 className="text-xl md:text-2xl font-black text-foreground uppercase group-hover:text-primary transition-colors mb-1 tracking-tight">{exp.role}</h3>
                      <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest">{exp.company}</p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                       {exp.tech.map((t) => (
                         <span key={t} className="px-3 py-1 bg-primary/5 border border-primary/20 rounded-full text-[10px] font-mono text-primary uppercase font-bold tracking-widest">
                            {t}
                         </span>
                       ))}
                    </div>
