  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-100 transition-all duration-1000 cubic-bezier(0.16, 1, 0.3, 1) ${
        isScrolled
          ? "bg-background/60 backdrop-blur-2xl border-b border-border/50 py-6"
      ...
      <div className="max-w-7xl mx-auto px-8 md:px-16 flex items-center justify-between overflow-hidden">
        ...
        <Link href="/" className={`group flex items-center gap-5 ${uniqueFont.className}`}>
          <div className="relative w-10 h-10 flex items-center justify-center">
            ...
            {/* Rotating Frames */}
            <div className="absolute inset-2 border border-border rounded-md group-hover:border-primary/50 transition-all duration-500 group-hover:rotate-90 group-hover:scale-110" />
            <div className="absolute inset-2 border border-border/50 rounded-md rotate-45 group-hover:border-sidebar-primary/50 transition-all duration-700 delay-100 group-hover:rotate-135" />
        ...
        <nav className={`hidden md:flex items-center gap-12 ${uniqueFont.className}`}>
          <div className="flex items-center gap-10 border-r border-border/50 pr-12">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.label}
                href={link.href}
        ...
        <button className="md:hidden group flex flex-col items-end gap-1.5 p-2 bg-muted/50 rounded-lg border border-border glass-card">
